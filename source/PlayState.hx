...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(songLowercase)
		{
			...
			case 'let-me-hear-you-squawk' | 'pecking-order' | 'stand-up':
		...
		switch(stageCheck)
		{
			...
			case 'touchsomegrass':
				{
					defaultCamZoom = 0.5;
					curStage = 'touchsomegrass';
					var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('SkyBackground'));
					...
					var hill1:FlxSprite = new FlxSprite().loadGraphic(Paths.image('HillsFarthestBack'));
					hill1.antialiasing = FlxG.save.data.antialiasing;
					hill1.screenCenter();
					hill1.active = false;
					hill1.scale.set(0.5, 0.5);
					hill1.scrollFactor.set(0.5, 0.5);
					add(hill1);

					var hill2:FlxSprite = new FlxSprite().loadGraphic(Paths.image('HillsBack'));
					hill2.antialiasing = FlxG.save.data.antialiasing;
					hill2.screenCenter();
					hill2.active = false;
					hill2.scale.set(0.5, 0.5);
					hill2.scrollFactor.set(0.7, 0.7);
					add(hill2);

					var floor:FlxSprite = new FlxSprite().loadGraphic(Paths.image('HillGround'));
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			if (curStage == 'touchsomegrass')
				{
					var tree:FlxSprite = new FlxSprite(-1000).loadGraphic(Paths.image('FgTree'));
					tree.antialiasing = FlxG.save.data.antialiasing;
					tree.screenCenter();
					tree.active = false;
					tree.scale.set(0.5, 0.5);
					tree.scrollFactor.set(1.1, 1.1);
					add(tree);

					var bush:FlxSprite = new FlxSprite(1000).loadGraphic(Paths.image('FgBush'));
					bush.antialiasing = FlxG.save.data.antialiasing;
					bush.screenCenter();
					bush.active = false;
					bush.scale.set(0.5, 0.5);
					bush.scrollFactor.set(1.15, 1.15);
					add(bush);
				}
		}

		eggh = new FlxSprite(-271, 200);
		eggh.frames = Paths.getSparrowAtlas('EggCutscene_Anim','shared',true);
		eggh.animation.addByPrefix('idle', 'EggAnimation instance 1', 24);
		eggh.animation.addByPrefix('fuckyou', 'EgggAnimation instance 1', 24);
		eggh.alpha = 0;
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
		{
			black2 = new FlxSprite(-1000, -1000).makeGraphic(FlxG.width * 4, FlxG.height * 4, FlxColor.BLACK);
			black2.scrollFactor.set();
			add(black2);
			camHUD.visible = false;
			new FlxTimer().start(0.1, function(fade:FlxTimer)
			{
					new FlxTimer().start(0.3, function(dialogue2:FlxTimer)
					{	
						if (dialogueBox != null)
						{
							inCutscene = true;
	...
	function startCountdown():Void
	{
		...
		if (SONG.song.toLowerCase() == 'pecking order')
			{
				remove(gf);
				gf = new Character(193, -19, 'gf-rock2');
				add(gf);
				remove(gf);
				gf = new Character(193, -19, 'gf-rock');
				add(gf);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (Std.isOfType(songNotes[4], Bool) && gottaHitNote)
				...
				if (Std.isOfType(songNotes[4], Bool))
					{
		...
		if (SONG.song.toLowerCase() == 'pecking order')
			{
				switch (curBeat)
				{
					case 28:
						...
						gf = new Character(193, -19, 'gf-rock2');
						add(gf);
						gf.playAnim('wheredidyougetthis');
						new FlxTimer().start(1.5, function(tmr:FlxTimer)
						if (gf.animation.curAnim.finished)
						{
							canPause = true;
							remove(gf);
							gf = new Character(193, -19, 'gf-rock3');
							add(gf);
					...
					case 160:
						dad.alpha = 0;
						eggh.alpha = 1;
						eggh.animation.play('idle');
						new FlxTimer().start(3.6, function(tmr:FlxTimer)
						{
							eggh.animation.play('fuckyou');
						});
			...
			if (SONG.song.toLowerCase() == 'stand up')
				{
					...
					{
                      case 10:
						eggh2.animation.play('breakout');
						new FlxTimer().start(5, function(tmr:FlxTimer)
							if (eggh2.animation.curAnim.finished)
							{
								remove(eggh2);
								dad.alpha = 1;
							});
